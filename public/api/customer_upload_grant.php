...
try {
    if ($revoke || $hours === 0) {
        ...
        $stmt->execute([$fileId]);
        add_file_log($pdo, $fileId, (int) $user['id'], 'Müşteri evrak yükleme izni kapatıldı');
        json_response([
            'ok' => true,
    ...
    add_file_log(
        ...
        $fileId,
        (int) $user['id'],
        'Müşteri evrak yükleme izni açıldı (' . $label . ', bitiş: ' . date('d.m.Y H:i', strtotime($until)) . ')'
    );
    ...
    json_error('İzin kaydedilemedi. Veritabanı güncellemesi gerekebilir.');
}
