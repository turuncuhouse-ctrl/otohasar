...
function loadHeic2Any() {
    ...
    heic2anyPromise = new Promise(function(resolve, reject) {
        ...
        script.src = '/assets/js/heic2any.min.js?v=14';
        script.async = true;
...
function normalizeUploadFile(file, index) {
    ...
    if (file.name === uploadName) return file;
    return new File([file], uploadName, {
        type: file.type || 'image/jpeg',
        lastModified: file.lastModified || Date.now()
    });
}

function createUploadPreviewCard(previewEl, file) {
...
function uploadDocuments(opts) {
    ...
    return prepareUploadFiles(opts.files).then(function(files) {
        ...
        if (files.length > maxFiles) {
            return Promise.reject({ error: 'En fazla ' + maxFiles + ' fotoğraf seçebilirsiniz' });
        }

        var cards = files.map(function(file) {
            return createUploadPreviewCard(previewEl, file);
        });
        var uploaded = [];
        var errors = [];
        var index = 0;
        ...
        function uploadNext() {
            ...
            var file = normalizeUploadFile(files[index], index);
            var card = cards[index];
            var current = index + 1;
            var total = files.length;
            index += 1;

            setCardProgress(card, 8, current + '/' + total + ' yükleniyor…');
            ...
            return apiUpload(uploadUrl, formData, function(percent) {
                ...
                errors.push(uploadLabel(file) + ': ' + message);
