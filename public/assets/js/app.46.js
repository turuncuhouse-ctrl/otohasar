...
function isLikelyImageFile(file) {
    ...
    var name = (file.name || '').toLowerCase();
    if (/\.(jpe?g|png|webp|heic|heif|gif|bmp)$/.test(name)) return true;
    if (!type || type === 'application/octet-stream') {
        // Uzantısız / bilinmeyen: görsel say (kamera çekimleri)
        return !/\.(pdf|docx?|xlsx?)$/.test(name);
    }
    return false;
}
...
function prepareUploadFiles(fileList) {
    var files = snapshotInputFiles(fileList).filter(function(f) {
        return isDocumentFile(f) || isLikelyImageFile(f);
    });
    if (!files.length) {
        return Promise.reject({ error: 'Dosya seçilmedi — JPEG/PNG/WebP, PDF, Word veya Excel seçin' });
    }
...
function appendFileToForm(formData, file, index) {
    ...
    if (isDocumentFile(file)) {
        ...
        } else if (isOfficeFile(file) && !/\.(docx?|xlsx?)$/i.test(uploadName)) {
            var t = (file.type || '').toLowerCase();
            if (t.indexOf('spreadsheet') !== -1 || t.indexOf('excel') !== -1) {
                uploadName = 'belge_' + (index + 1) + '.xlsx';
            } else {
                uploadName = 'belge_' + (index + 1) + '.docx';
            }
        }
...
function uploadDocuments(opts) {
    ...
    return prepareUploadFiles(opts.files).then(function(files) {
        ...
        function uploadNext() {
            ...
            setCardProgress(card, 8, current + '/' + total + (isDocumentFile(sourceFile) ? ' hazırlanıyor…' : ' küçültülüyor…'));

            return prepareImageForUpload(sourceFile).then(function(file) {
                if (!isDocumentFile(file) && file.size > 20 * 1024 * 1024) {
                    return Promise.reject({ error: 'Fotoğraf 20MB üzerinde kaldı' });
                }
                if (isDocumentFile(file) && file.size > 20 * 1024 * 1024) {
                    return Promise.reject({ error: 'Dosya 20MB limitini aşıyor' });
                }
...
function startUploadFromInput(input, opts, category) {
    ...
    readInputFiles(inputEl).then(function(files) {
        ...
        showToast(files.length + ' dosya seçildi, yükleniyor…', 'success');
...
var UPLOAD_ACCEPT_IMAGES = 'image/jpeg,image/png,image/webp,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.heic,.heif';
var UPLOAD_ACCEPT_OFFICE = 'application/pdf,.pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.doc,.docx,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,.xls,.xlsx';

function closeUploadSourceSheet(sheet) {
    if (!sheet) return;
    sheet.hidden = true;
...
function ensureUploadSourceSheet(opts) {
    var sheet = document.getElementById('uploadSourceSheet');
    if (sheet) return sheet;

    sheet = document.createElement('div');
    sheet.id = 'uploadSourceSheet';
    sheet.className = 'upload-source-sheet';
    sheet.hidden = true;
    sheet.innerHTML =
        '<div class="upload-source-backdrop" data-sheet-close="1"></div>' +
        '<div class="upload-source-panel" role="dialog" aria-modal="true" aria-labelledby="uploadSourceTitle">' +
        '  <h4 id="uploadSourceTitle">Evrak yükle</h4>' +
        '  <p class="upload-source-sub" id="uploadSourceSub">Nasıl yüklemek istersiniz?</p>' +
        '  <button type="button" class="upload-source-btn" data-source="camera">📷 Kamera ile çek</button>' +
        '  <button type="button" class="upload-source-btn" data-source="gallery">🖼️ Galeriden fotoğraf</button>' +
        '  <button type="button" class="upload-source-btn upload-source-btn-doc" data-source="document">📄 PDF / Word / Excel</button>' +
        '  <button type="button" class="upload-source-cancel" data-sheet-close="1">Vazgeç</button>' +
        '</div>' +
        '<input type="file" id="uploadSrcCamera" class="upload-source-file" accept="image/*" capture="environment">' +
        '<input type="file" id="uploadSrcGallery" class="upload-source-file" accept="' + UPLOAD_ACCEPT_IMAGES + '" multiple>' +
        '<input type="file" id="uploadSrcDocument" class="upload-source-file" accept="' + UPLOAD_ACCEPT_OFFICE + '" multiple>';
    document.body.appendChild(sheet);

    sheet.addEventListener('click', function(e) {
        if (e.target && e.target.getAttribute('data-sheet-close')) {
            closeUploadSourceSheet(sheet);
        }
    });

    sheet.querySelectorAll('[data-source]').forEach(function(btn) {
        btn.addEventListener('click', function() {
            var source = btn.getAttribute('data-source');
            var category = sheet.dataset.category || 'diger';
            var inputId = source === 'camera' ? 'uploadSrcCamera'
                : (source === 'document' ? 'uploadSrcDocument' : 'uploadSrcGallery');
            var input = document.getElementById(inputId);
            closeUploadSourceSheet(sheet);
            if (!input) return;
            input.value = '';
            input.onchange = function() {
                startUploadFromInput(input, sheet._uploadOpts || opts, category);
            };
            input.click();
        });
    });
...
function openUploadSourceSheet(opts, category, label) {
    var sheet = ensureUploadSourceSheet(opts);
    sheet._uploadOpts = opts;
    sheet.dataset.category = category || 'diger';
    var title = document.getElementById('uploadSourceTitle');
    var sub = document.getElementById('uploadSourceSub');
    if (title) title.textContent = (label || 'Evrak') + ' yükle';
    if (sub) {
        sub.textContent = category === 'hasar_foto'
            ? 'Fotoğraf için kamera/galeri; poliçe vb. için PDF seçin.'
            : 'Fotoğraf çekin/seçin veya PDF/Word/Excel yükleyin (örn. kasko poliçesi).';
    }
    sheet.hidden = false;
...
function bindCategoryUpload(opts) {
    ensureUploadSourceSheet(opts);
    document.querySelectorAll(opts.gridSelector + ' .category-card').forEach(function(card) {
        var legacy = card.querySelector('.cat-input');
        if (legacy) {
            legacy.remove();
        }
        card.classList.add('category-card-pick');
        if (!card.hasAttribute('role')) card.setAttribute('role', 'button');
        if (!card.hasAttribute('tabindex')) card.setAttribute('tabindex', '0');

        function open() {
            var labelEl = card.querySelector('.cat-label');
            openUploadSourceSheet(opts, card.dataset.category, labelEl ? labelEl.textContent.trim() : '');
        }
